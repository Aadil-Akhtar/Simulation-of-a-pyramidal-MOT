...
N=13000  #number of atoms

z=np.arange(start=0,stop=1300,step=0.1)  #so this is z, taking step = 0.1
...
def a_exp(g_u):
    a_z_val=[]
    for j in range(len(z)):
        a_z =  (n_l[j]*R(0,2,+1, (j*10e-3) ,g_u)*tau) /( R(0,2,+1, (j*10e-3) ,g_u) + tau)  -  (n_l[j]*R(0,0,-1, (j*10e-3) ,g_u)*tau) /( R(0,0,-1, (j*10e-3) ,g_u) + tau)   
        a_z_val.append(a_z)
    return a_z_val
